...
class DataLoadHandler():
    ...
    def __init__(self, data_path = 'annotated_sentences_all.json', test_size = 0.25):

        ess_df = pd.read_json('annotated_sentences_all.json')
        ess_df = ess_df[['sent-text', 'sent-class']]
        web_df = pd.read_json('annotated_sentences.json')
